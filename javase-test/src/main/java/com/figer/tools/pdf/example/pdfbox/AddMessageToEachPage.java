...
public class AddMessageToEachPage
{
  ...
  public void addMessageOnGivenLocation(String file, String message, String  outfile , float x, float y) throws IOException
  {
    try (PDDocument doc = PDDocument.load(new File(file)))
    {
      float fontSize = 36.0f;
      ...
      for( PDPage page : doc.getPages() )
      {
        ...
        try (PDPageContentStream contentStream = new PDPageContentStream(doc, page, AppendMode.APPEND, true, true))
        {
          ...
          contentStream.setTextMatrix(Matrix.getTranslateInstance(pageWidth*x, pageHeight*y));
          contentStream.showText(message);
          contentStream.endText();
        }
      }
  ...
  public static void main(String[] args) throws IOException
  {
    ...
    {
      app.addMessageOnGivenLocation( args[0], args[1], args[2], 0.5f, 0.5f );
    }
  }
